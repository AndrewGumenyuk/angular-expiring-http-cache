 ...
 * @param {T} [initialValue] Optional. Initial value of cache before first source execution.
 */
export function createCache<T>(source: Observable<T>, updateWhen: Observable<any>, initialValue?: T) {
  const cacheSubject = new BehaviorSubject(initialValue);
 ...
 * @param {T} [initialValue] Optional. Initial value of cache before first source execution.
 */
export function createTimerCache<T>(expirationPeriod = defaultExpirationPeriod, initialValue?: T) {
  const cacheSubject = new BehaviorSubject<T>(initialValue);
  const cache = cacheSubject.asObservable();
  ...
  return (source: Observable<T>) =>
    Observable.timer(0, expirationPeriod)
      .switchMap(count => {
        ...
        return count === 0 ? next.subscribe() && cache : next;
      })
    .share();
}
